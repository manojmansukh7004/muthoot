...
const {RootCheck, FridaModule, OverlayDetection, IntegrityCheck} =
  NativeModules;

const expectedHash: string =
  'ef45546b48e43c88870d784a51a6d02b4b424d974087b9aaccd902b55049c81a';
...
const Splash: FC<SplashProps> = ({route}) => {
  ...
  // console.log("kkkkkkkk",employeeId);
  ...
  const onPressUpdate = () => {
    Linking.openURL(
      'https://drive.google.com/drive/folders/10e-z19ZDdbJH1U8wQgGTpduFLUEm09nN',
    );
  };
  ...
  const isRootedDevice = async () => {
    try {
      ...
      const hasOverlayPermission =
        await OverlayDetection.isOverlayPermissionGranted();
      const isSuspiciousOverlayRunning =
        await OverlayDetection.isSuspiciousOverlayRunning();
      const isEmulator = await DeviceInfo.isEmulator();
      const isIntact = await IntegrityCheck.checkIntegrity(
        VersionCheckData?.hashkey,
      );
      ...
      if (
        ...
        isSuspiciousOverlayRunning ||
        isEmulator
        // || !isIntact
      ) {
        setIsDeviceRooted(true);
      } else {
        FridaModule?.detectFrida()
          .then((isDetected: boolean) => {
    ...
    <View style={styles.container}>
      ...
      {VersionCheckData?.appVersionflag ? (
        ...
        <Card style={styles.card}>
          ...
          <Text style={styles.textCard}>
            Potential risks detected on your device, such as rooting, overlay
            permissions, running in an emulator, etc.
          </Text>
          <Text style={styles.textCard}>
            For your protection, certain features may be limited.
          </Text>
        </Card>
      ) : (
        <>
      ...
      <View
        ...
        <View
          ...
          <Text
            style={{color: Colors.LabelGrey, fontSize: FONT_SIZE.xs, top: 1}}>
            Powered by{' '}
          </Text>
          <View style={{width: '32%'}}>
            <Image
        ...
        <View
          ...
          <View style={{}}>
            <Text
              ...
              AnalyticsFox Softwares Pvt. Ltd.
            </Text>
          </View>
        </View>
      </View>
