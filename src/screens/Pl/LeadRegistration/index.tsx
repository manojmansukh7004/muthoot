...
import { useGetConstitution } from 'api/ReactQuery/PL/Master';
...
const LeadRegistration: FC<LeadRegistrationScreenProps> = ({
  ...
  console.log("constitutionTypeconstitutionType", constitutionType);
  ...
  useEffect(() => {
    ...
    if (GetLeadData && isMainApplicant) {
      ...
      setIsPreOffer(
        GetLeadData.isSulbThird ? 'Yes' : 'No'
      );
    }
  ...
  useEffect(() => {
    if (incomeStatus && isChanged) {
      setMonthlyIncome('');
      // setIsError()
    }
  }, [incomeStatus]);

  console.log("iiiiii", isChanged);
  ...
  // console.log("isPreOffer", isPreOffer);
  ...
  return (
    <WaveBackground
      ...
      <RadioButtonGroup
        ...
        options={isMainApplicant ? ['Existing Customer'] : [
          'Existing Employee',
          'Existing Customer',
          'None'
        ]}
...

      {
        ...
        <LabeledTextInput
          label={isMainApplicant && incomeStatus == 'Earning' ? "Income (Monthly)" : !isMainApplicant && incomeStatus == 'Earning' ? "Income (Monthly)" : "Income (Monthly) "}
