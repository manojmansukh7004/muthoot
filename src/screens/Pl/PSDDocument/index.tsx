...
} from 'api/ReactQuery/PL/Document';
import {
  docTypes,
  SavePSDRequest,
  getInvoiceDetailsWithIPBranchRequest
} from 'api/ReactQuery/PL/PSDDocument/types';
import {
  useGetPSD,
  useSavePSD,
  useGetInsuranceCompanyMaster,
  useGetInvoiceDetailsWithIPBranch
} from 'api/ReactQuery/PL/PSDDocument';
...
} from 'api/ReactQuery/PL/Relationship/types';
import {
  useGetRelationshipMaster,
} from 'api/ReactQuery/PL/Relationship';
...
const PSDDocument: FC<PSDDocumentScreenProps> = ({
  ...
  const [performaInvoiceUrlType, setPerformaInvoiceUrlType] = useState<string>('');
  ...
  const [RelationshipWithApplicant, setRelationshipWithApplicant] = useState<string>('');
  ...
  let isActive: boolean = useActive(isActiveArray
    // originalInvoice == 'Yes' && originalInsurance == 'Yes'? isOrignalInvoiceWithInsurance :
    //   originalInvoice == 'No' && originalInsurance == 'Yes'? isActivePerformaWithInsurance :
    ...
    const callback = (response: any, path: any, name: string) => {
      ...
      if (response) {
        ...
        type == 'Invoice' ? (
          ...
          setRetailInvoiceUrlType((response[0].type?.split("/")?.pop()) == 'pdf' ? 'pdf' : 'jpg')
        )
          : type == 'Proforma Invoice' ? (
            setPerformaInvoiceUrl(response[0]?.fileCopyUri),
            setPerformaInvoiceUrlType((response[0].type?.split("/")?.pop()) == 'pdf' ? 'pdf' : 'jpg')
          )
            : type == 'Insurance' ? (
              setInsuranceUrl(response[0]?.fileCopyUri),
              setInsuranceUrlType((response[0].type?.split("/")?.pop()) == 'pdf' ? 'pdf' : 'jpg')
            )
              : type == 'IP' ? (
                setIpUrl(response[0]?.fileCopyUri),
                setIpUrlType((response[0].type?.split("/")?.pop()) == 'pdf' ? 'pdf' : 'jpg')
              )
                : type == 'Gold Receipt' ? (
                  setGoldReceiptUrl(response[0]?.fileCopyUri),
                  setGoldReceiptUrlType((response[0].type?.split("/")?.pop()) == 'pdf' ? 'pdf' : 'jpg')
                )
                  : null
        ...
        convertImageFileToBase64(response[0]?.fileCopyUri)
          .then(base64Data => {
            if (base64Data) {
              ...
              setDocType((response[0].type?.split("/")?.pop()) == 'pdf' ? 'pdf' : 'jpg')
              setBase64Value(base64Data)
              setUploadingDocumentType(type)
            }
          })
...
// console.log("nnnnnnnnn",nomineeDOB);
  ...
  const SavePSDRequest: SavePSDRequest = {
    ...
    sumInsured: insuranceUrl == '' ? 0 : parseFloat(sumInsured) ? parseFloat(sumInsured) : null,
  ...
  useEffect(() => {
    if (GetPSDDetailData) {
      ...
      setOriginalInvoice((GetPSDDetailData?.isOriginalInvoice) ? 'Yes' : GetPSDDetailData?.isOriginalInvoice == null ? originalInvoice : 'No')
      setOriginalInsurance(GetPSDDetailData?.insuranceFilePath ? 'Yes' : (GetPSDDetailData?.isOriginalInsurance) ? 'Yes' : GetPSDDetailData?.isOriginalInsurance == null ? originalInsurance : 'No')
      ...
      setRelationshipWithApplicant(GetPSDDetailData?.applicantRelationship || '')
      setNomineeGender(GetPSDDetailData?.gender || '')
      setNomineeMaritalStatus(GetPSDDetailData?.martialStatus || '')
      setNomineeDOB(GetPSDDetailData?.dob ? new Date(GetPSDDetailData?.dob) : '')
      ...
      setGoldReceiptUrl(GetPSDDetailData?.goldReceiptUrl == null ? '' : GetPSDDetailData?.goldReceiptUrl || '')
  ...
  return (
    <WaveBackground
      ...
      {/* <LabeledRadioButtonGroup
        ...
        disabled={isViewOnly ? isViewOnly : originalInvoice == 'Yes' ? retailInvoiceUrl !== '' : originalInvoice == 'No' ? performaInvoiceUrl !== '' : false}
      /> */}
      ...
      {/* {originalInvoice == 'Yes' ?
        <>
          ...
          <View style={[style.FinalLabel, { paddingHorizontal: 20 }]}>
            ...
            <View style={{ width: '60%' }}>
              <Text style={[style.StaticLabel, { flexWrap: 'wrap' }]}>{vehicleModel}</Text>
            </View>
          </View>
      ...
      {/* <>
        ...
        <LabeledRadioButtonGroup
          ...
          inLine
          // disabled={isViewOnly}
          disabled={isViewOnly ? isViewOnly : originalInsurance == 'Yes' ? insuranceUrl !== '' :  false}
          ...
          <>
            ...
            <View style={style.dobContainer}>
              ...
              <TouchableOpacity style={[style.inputbox, { borderColor: insuranceFromCalendarOpen ? Colors.Black : Colors.LightGrey }]}
                disabled={isViewOnly}
                onPress={() => { setInsuranceFromCalendarOpen(!insuranceFromCalendarOpen), setIsChanged(true) }}
              >
            ...
            <View style={style.dobContainer}>
              ...
              <TouchableOpacity style={[style.inputbox, { borderColor: insuranceTocalendarOpen ? Colors.Black : Colors.LightGrey }]}
                disabled={isViewOnly}
                onPress={() => { setInsuranceToCalendarOpen(!insuranceTocalendarOpen) ,setIsChanged(true)}}
              >
      ...
      <>
        ...
        <View style={style.dobContainer}>
          <View style={{ flexDirection: 'row' }}>
            <Text style={[style.labelText, { color: nomineeDOBOpen ? Colors.Black : Colors.LabelGrey }]}>
              {'Date Of Birth'}
            </Text>
            {isNomineeMandatory && <Icon name="pointed-star" />}
          </View>
          <TouchableOpacity style={[style.inputbox, { borderColor: nomineeDOBOpen ? Colors.Black : Colors.LightGrey }]}
            disabled={isViewOnly}
            onPress={() => { setNomineeDOBOpen(!nomineeDOBOpen) }}
          >
            <Text style={{ color: nomineeDOB ? Colors.Black : Colors.LabelGrey }}>
              {nomineeDOB ? moment(nomineeDOB).format('DD-MM-YYYY').toString() : null}
            </Text>
          </TouchableOpacity>
        </View>
        ...
        {goldReceiptUrl == '' &&
          <View
            ...
            <TouchableOpacity
              ...
              disabled={isViewOnly}
              onPress={() => { handleOnPress('Gold Receipt') }}>
              <Icon name="upload" />
            </TouchableOpacity>
            <TouchableOpacity
              style={{
              ...
              disabled={isViewOnly}
              onPress={() => { handleLaunchCamera('Gold Receipt') }}>
              <Icon name="capture" />
            </TouchableOpacity>
          </View>}
        ...
        onPress={() => {
          ...
          isChanged ?
            insuranceUrl && (
              ...
              (Number(premiumAmount?.replaceAll(',', '')) < (Number(insurenceCaping) - Number(100)))
              //  ||
              ...
              originalInsurance == 'Yes' && Number(sumInsured) <= (Number(0.9) * Number(GetPSDDetailData?.exShowRoomPrice)) ? (
                setErrorMsg(`Sum Insured should not be less than or equal to 90% of Ex-showroom price`),
                setCommonPopUPVisible(true)
              )
                :
                originalInvoice == 'Yes' && Number(exShoroomPrice) < (Number(GetPSDDetailData?.exShowRoomPrice)) ? (
                  setErrorMsg(`Ex-Showroom Price as Per Invoice should be greater than or equal to Ex-showroom price`),
                  setCommonPopUPVisible(true)
                )
                  :
                  originalInvoice == 'No' && Number(exShowroomPriceAsPerPerforma) < (Number(GetPSDDetailData?.exShowRoomPrice)) ? (
                    setErrorMsg(`Ex-Showroom Price as Per Invoice should be greater than or equal to Ex-showroom price`),
                    setCommonPopUPVisible(true)
                  )
                    :
                    IPPaidBranch == 'IP Paid at Dealer' &&
                      ...
                      IPPaidBranch == 'IP Paid at Branch' &&
                        ...
                        setErrorMsg(`IP Paid amount not match with Branch IP amount, Please try again!!!`),
                        setCommonPopUPVisible(true))
                        :
                        IPPaidBranch == 'IP Paid at Both' &&
                          Number(temp) != Number(downPayment) ?
