...
import {updateMasterLogin} from 'api/Axios/TwoWheelerBaseurl';
...
const MasterLogin: FC<MasterLoginScreenProps> = ({navigation, route}) => {
  ...
  const handleProceed = async () => {
    if (productType === 'New Two-Wheeler') {
      if (appId.startsWith('MU')) {
        ...
        //  await Keychain.setGenericPassword('ismasterLogin', 'true');
        await updateMasterLogin('true');
        navigation.navigate('TwoWheelerStack', {
          screen: 'LoanSummary',
          params: {ismasterLogin: true},
        });
      } else {
  ...
  useEffect(() => {
    if (MasterLoginData) {
      ...
      MasterLoginData?.isNextEnable ? handleProceed() : null;
    }
  }, [MasterLoginData]);
