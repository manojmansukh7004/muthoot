...
import {
  ...
  useMerchantStatus,
  useOneRupessMandate
} from 'api/ReactQuery/TwoWheeler/Wrapper';
import { SaveRepaymentDetailsRequest, deleteNachRequest, UploadNachRequest, saveCashRequest, GetPhysicalNachRequest } from 'api/ReactQuery/TwoWheeler/Repayment/types';
import {
  ...
  useEmandateBankList,
  useEsignBankList
} from 'api/ReactQuery/TwoWheeler/Repayment';
import {
...
} from 'api/ReactQuery/TwoWheeler/Wrapper/types';
...
import { useGetLead } from 'api/ReactQuery/TwoWheeler/Lead';
...
const RepaymentDetails: FC<RepaymentDetailsScreenProps> = ({
  ...
  const [isVisibleCashOnly, setIsVisibleCashOnly] = useState<boolean>(false);
  ...
  const CreateMandateVpaRequest: CreateMandateVpaRequest = {
    ...
    debitStartDate: moment(new Date(debitStartDate)).format('YYYY-MM-DD'),
    // debitStartDate,
    amount:
      // '1.00',
      (appData?.emi) || '',
    payerName: ValidateVPAData?.resData?.verifiedName || '',
    payerVpa: VPA,
    // ValidateVPAData?.data?.payerVpa || '',
    service: 'salesApp',
    isNachReactivation: isNachReactivation

  };

  const OneRupessMandateRequest: OneRupessMandateRequest = {
    appId: applicantId
  };

  const MerchantStatusRequest: MerchantStatusRequest = {
    appId: applicantId
  };
  ...
  const [
    MerchantStatus,
    { data: MerchantStatusData, isLoading: MerchantStatusIsLoading },
  ] = useMerchantStatus(MerchantStatusRequest);

  const [
    OneRupessMandate,
    { data: OneRupessMandateData, isLoading: OneRupessMandateIsLoading },
  ] = useOneRupessMandate(OneRupessMandateRequest);

  const [
    GetVpaStatus,
    { data: GetVpaStatusData, isLoading: GetVpaStatusIsLoading },
  ] = useGetVpaStaus(`?appId=${applicantId}`);
  ...
  useEffect(() => {
    if (GetEnachResponseData) {
      console.log("GetEnachResponseData", GetEnachResponseData);
  ...
  useEffect(() => {
    console.log("ValidateVPAData", ValidateVPAData);

    if (ValidateVPAData && ValidateVPAData?.responseStatus == 'Success') {
  ...
  useEffect(() => {
    if (GetCashDetailsData) {
      ...
      setIsVisibleCashOnly(GetCashDetailsData.isVisibleCashOnly)
  ...
  useEffect(() => {
    if (CreateMandateVpaData) {
      // manoj
      console.log("CreateMandateVpaData", CreateMandateVpaData);
      GetRepaymemtDetails.mutateAsync();
      GetEnachResponse.mutateAsync()
      // GetVpaStatus.mutateAsync();
    }
  ...
  useEffect(() => {
    if (GetVpaStatusData) {
      // manoj
      console.log("GetVpaStatusData", GetVpaStatusData);

      GetVpaStatusData?.responseStatus == 'Success' ?
        OneRupessMandate.mutateAsync() : null
  ...
  useEffect(() => {
    if (OneRupessMandateData) {
      console.log("OneRupessMandateData", OneRupessMandateData);

      // manoj

      // setTimeout(() => {
      //   console.log("in timeouttttt");
      MerchantStatus.mutateAsync()
      // }, 1000);
      // OneRupessMandateData?.resData?.status == 'Success'?
      // MerchantStatus.mutateAsync(): null

    }
  }, [OneRupessMandateData]);
  ...
  // console.log("esignBankDropdownList", imageUrl);
  ...
  let isActveCash: boolean = useActive([]);
  ...
  return (
    <WaveBackground
      loading={[
        ...
        GetVpaStatusIsLoading,
        MerchantStatusIsLoading,
        OneRupessMandateIsLoading
      ]}
      ...
      <FlatList
        ...
        data={isVisibleCashOnly ? RepaymentMethodOptions.filter(item => item == 'Cash') : isCashVisible ? RepaymentMethodOptions : RepaymentMethodOptions.filter(item => item !== 'Cash')}
        ...
        GetRepaymemtDetailsData?.status && (
          ...
          <View style={{ marginVertical: 15 }}>
            <Button
              ...
              onPress={() => {
                (selectedRepaymentMethod == 'UPI Autopay' && GetRepaymemtDetailsData?.status !== 'SUCCESS') ?
                  GetVpaStatus.mutateAsync()
                  :
                  GetEnachResponse.mutateAsync()
              }}
