...
import {useGetSanctionLetterDetails, useSherLockCalling} from 'api/ReactQuery/TwoWheeler/Lead';
import {sherLockCallingRequest} from 'api/ReactQuery/TwoWheeler/Lead/types';
...
const LoanOffer: FC<LoanOfferScreenProps> = ({navigation, route}) => {
  const {applicantId,guarantorId, SetMainApplicant} = useApplicantDetails();
  ...
  const [isGuarantorMandatory, setIsGuarantorMandatory] =useState<boolean>(false);
  const [isNextEnable, setIsNextEnable] = useState<boolean>(false);
  const [addGuarantorVisible, setAddGuarantorVisible] = useState<boolean>(false);
  const [isRetryButtonVisible, setIsRetryButtonVisible] = useState<boolean>(false);

  const sherLockCallingRequest : sherLockCallingRequest={
  ...
  const [
    SherLockCalling,
    {
      data: SherLockCallingData,
      isLoading: SherLockCallingIsLoading,
    },
  ] = useSherLockCalling(sherLockCallingRequest);

  useEffect(()=>{
    if(SherLockCallingData){
      GetSoftOfferDetails.mutateAsync()
    }

  },[SherLockCallingData])

  useEffect(() => {
    if (GetSoftOfferDetailsData) {
      console.log("GetSoftOfferDetailsData",GetSoftOfferDetailsData);
      
      setLoanAmount(GetSoftOfferDetailsData.loanAmount?.toString() || '');
      setRateofInterest(GetSoftOfferDetailsData.roi?.toString() || '');
      setEMIAmount(GetSoftOfferDetailsData.emiAmount?.toString() || '');
      setCustomerName(GetSoftOfferDetailsData.customerName || '');
      setTenure(GetSoftOfferDetailsData.tenure?.toString() || '');
      setIsGuarantorMandatory(GetSoftOfferDetailsData.isGuarantorMandatory || false,);
      setPopUPVisible(GetSoftOfferDetailsData.isNextEnable? false : GetSoftOfferDetailsData.nipRule ?  true : GetSoftOfferDetailsData.incomeRule  );
      setPopUPMsg(GetSoftOfferDetailsData.isNextEnable? '': GetSoftOfferDetailsData.nipRule ? GetSoftOfferDetailsData.nipMessage : GetSoftOfferDetailsData.incomeMessage  );
      setIsNextEnable(GetSoftOfferDetailsData.isNextEnable || false);
      setAddGuarantorVisible(GetSoftOfferDetailsData.addGuarantorVisible);
      setIsRetryButtonVisible(GetSoftOfferDetailsData.isRetryButtonVisible || false);

    }
  }, [GetSoftOfferDetailsData]);
  ...
  return (
    <WaveBackground loading={[GetSanctionLetterDetailsIsLoading, SherLockCallingIsLoading]} PureScreen>
      <View style={{marginTop: '7%', paddingHorizontal: '5%'}}>
        ...
        <View
          ...
          {GetSoftOfferDetailsData?.isNextEnable && (
          ...
          {GetSoftOfferDetailsData?.addGuarantorVisible &&
            !GetSoftOfferDetailsData.isNextEnable && (
              <Text style={styles.MessageText}>
