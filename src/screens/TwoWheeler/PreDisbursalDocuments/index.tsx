...
import { PrePosDto } from 'api/ReactQuery/TwoWheeler/Master/types';
import { useGetPreDocumentsDetail } from 'api/ReactQuery/TwoWheeler/Master';
import {
      ...
      <View style={{ borderBottomWidth: .2, paddingBottom: 15, marginTop: 5 }}>
        {<View style={{ marginBottom: 5 }}>
          <LabeledUploadMultiDocument
            ...
            disabled={isViewOnly}
            buttonDisable={(values.documentType == 'PAN' || values.documentType == 'Profile Image' || values.documentType == 'Aadhaar' || values.documentType == "CLI Document" || values.documentType == "PLI Document") ? values.filePath : values.documentType == 'OVD' || values.documentType == 'OCR' ? (values.filePath && values.filePath2) : Number(values?.multipleFilePath?.length) >= Number(values?.count)}
          />
        </View>
        }
                      ...
                      {item.fileType == 'jpg' ?
                        <RenderImage
                          ...
                          isNonDeletable={
                            ...
                            values.documentType === 'Account Statement' ||
                            values.documentType === 'Bank Passbook' 
                          }
