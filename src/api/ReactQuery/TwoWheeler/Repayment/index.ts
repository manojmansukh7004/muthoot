    ...
    async () => {
      ...
      if (response.error) {
        ...
        if (response?.data?.status === 'SUCCESS') {
          useShowFlashMessage('success', response?.data?.repaymentType ==  'Physical Nach' ? 'Congratulation your Nach setup has been moved forward to disbursement team!': 'Congratulation your Nach setup is done successfully!');
...
export const useGetRepaymentStatus = (
  payload: getRepaymentStatusRequest,
) => {
  const mutation = useMutation<getRepaymentStatusResponse>(
    ['Repayment', payload],
    async () => {
      const response = await Repayment.GetRepaymentStatus(payload);
      if (payload?.type !== 'Cancel' && response.error) {
        useShowFlashMessage('warning', response.message);
      }
      return response.data;
    },
