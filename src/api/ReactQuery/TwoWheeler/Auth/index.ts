...
export const useGetUpdatePassword = (id) => {
  const mutation = useMutation<GetUpdatePasswordResponse>(
    ['Auth'],
    async () => {
      const response = await Auth.GetUpdatePassword(id);
      if (response.error) {
        useShowFlashMessage('warning', response.message);
      }
...
export const useMasterLogin = (payload: MasterLoginRequest) => {
  const mutation = useMutation<MasterLoginResponse>(['Auth', payload], async () => {
    const response = await Auth.MasterLogin(payload);
    if (response.error) {
      useShowFlashMessage('warning', response.message);
    }
    return response.data;
